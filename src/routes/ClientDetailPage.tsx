...
export default function ClientDetailPage() {
  ...
  const [charts, setCharts] = useState<Record<string, LineSeries[]>>({}); const [apps, setApps] = useState<BarListItem[]>([]); const [uploadApps, setUploadApps] = useState<BarListItem[]>([]); const [packetApps, setPacketApps] = useState<BarListItem[]>([]); const [events, setEvents] = useState<string[][]>([]); const [eventsLoading, setEventsLoading] = useState(true);
  useEffect(() => {
    let cancelled = false;
    ...
    void q(`unpoller_client_uptime_seconds{name="${name}"}`).then(async identity => {
      ...
      // Upload-side DPI is appended last and located by index: the wireless-only
      // requests below shift every position after them.
      ...
      // Upload-side DPI, appended after the wireless-only requests so their
      // indices (4-7) are untouched; located by captured index.
      const dpiTxAt = requests.length; requests.push(q(`topk(10, sum by (application) (unpoller_client_dpi_transmit_bytes{${selector},application!="TOTAL"}))`));
      /* DPI packet counters are gauges (windowed counts, not monotonic
         counters), so they are read directly — wrapping them in rate() would
         divide a per-window count by the scrape step and report nonsense. */
      const dpiPktsAt = requests.length; requests.push(q(`topk(10, sum by (application) (unpoller_client_dpi_receive_packets{${selector},application!="TOTAL"}) + sum by (application) (unpoller_client_dpi_transmit_packets{${selector},application!="TOTAL"}))`));
      ...
      setUploadApps(result[dpiTxAt].filter(x => Number.isFinite(x.value) && x.value > 0).map(x => ({ label: x.labels?.application ?? 'Unknown', value: x.value })).sort((a, b) => b.value - a.value));
      setPacketApps(result[dpiPktsAt].filter(x => Number.isFinite(x.value) && x.value > 0).map(x => ({ label: x.labels?.application ?? 'Unknown', value: x.value })).sort((a, b) => b.value - a.value));
  ...
  }, [name, tr.earliest, tr.range, tr.refreshKey, tr.step]);
