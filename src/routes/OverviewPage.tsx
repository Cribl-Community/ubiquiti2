...
export default function OverviewPage() {
  ...
  const [values, setValues] = useState({ aps: 0, switches: 0, gateways: 0, users: 0, guests: 0, iots: 0, latency: 0, uptime: 0, adopted: 0, offline: 0, pending: 0 });
  ...
  useEffect(() => { void Promise.all([latestMetric('sum(unpoller_site_aps)',7),latestMetric('sum(unpoller_site_switches)',6),latestMetric('sum(unpoller_site_gateways)',1),latestMetric('sum(unpoller_site_users{status="ok"})',79),latestMetric('sum(unpoller_site_guests{status="ok"})',0),latestMetric('sum(unpoller_site_iots{status="ok"})',1),latestMetric('1000 * avg(unpoller_site_latency_seconds)',18),latestMetric('max(unpoller_site_uptime_seconds)',1296000),latestMetric('sum(unpoller_site_adopted)',0),latestMetric('sum(unpoller_site_disconnected)',0),latestMetric('sum(unpoller_site_pending)',0)]).then(([aps,switches,gateways,users,guests,iots,latency,uptime,adopted,offline,pending]) => setValues({ aps,switches,gateways,users,guests,iots,latency,uptime,adopted,offline,pending })); void Promise.all([queryMetric('sum(unpoller_site_receive_rate_bytes{status="ok",subsystem=~"wan|lan"})',tr.step,tr.earliest),queryMetric('sum(unpoller_site_transmit_rate_bytes{status="ok",subsystem=~"wan|lan"})',tr.step,tr.earliest),queryMetric('sum(unpoller_site_users{status="ok"})',tr.step,tr.earliest),queryMetric('sum(unpoller_site_guests{status="ok"})',tr.step,tr.earliest),queryMetric('sum(unpoller_site_iots{status="ok"})',tr.step,tr.earliest),queryMetric('rate(unpoller_site_intenet_drops_total[5m])',tr.step,tr.earliest)]).then(([r,t,u,g,i,d]) => { if(r.length) setReceive(r); if(t.length) setTransmit(t); if(u.length) setUserSeries(u); if(g.length) setGuestSeries(g); if(i.length) setIotSeries(i); if(d.length) setDropSeries(d); }); }, [tr.earliest, tr.range, tr.refreshKey, tr.step]);
  useEffect(() => {
  ...
  const cards = [['Access points', values.aps, ''], ['Switches', values.switches, ''], ['Gateways', values.gateways, ''], ['Clients', values.users, ''], ['Guests', values.guests, ''], ['IoT devices', values.iots, ''], ['WAN latency', `${values.latency.toFixed(1)} ms`, 'blue'], ['Uplink uptime', `${Math.floor(values.uptime / 86400)}d ${Math.floor(values.uptime / 3600) % 24}h`, ''], ['Devices adopted', values.adopted, ''], ['Devices offline', values.offline, values.offline > 0 ? 'blue' : ''], ['Pending adoption', values.pending, values.pending > 0 ? 'blue' : '']];
