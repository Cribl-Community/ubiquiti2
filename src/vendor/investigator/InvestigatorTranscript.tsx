...
import { Button } from '../capra';
import type { LoopEvent } from '../agent-loop';
import { isSessionExpiredError, type AgentToolCall } from '../agent';
import type {
  RunSearchUi,
  SummaryUi,
  ToolExecutionResult,
...
function renderAssistantMarkdown(text: string): ReactNode[] {
  ...
  for (const part of parts) {
    ...
    for (const para of paras) {
      ...
      if (!trimmed) continue;
      // ATX headings: `# Title` through `###### Title`. The agent
      // writes markdown-style section heads in free-form text, and
      // without this they render as literal hash marks. A heading
      // that shares its paragraph with prose (the usual case) keeps
      // the rest as a follow-on paragraph.
      const heading = /^(#{1,6})\s+(.+)$/.exec(trimmed.split('\n')[0]);
      if (heading) {
        const level = Math.min(heading[1].length, 3);
        nodes.push(
          <div key={`h-${nodeKey++}`} className={s.assistantHeading} data-level={level}>
            {renderInline(heading[2])}
          </div>,
        );
        const rest = trimmed.split('\n').slice(1).join('\n').trim();
        if (rest) {
          const restParsed = tryParseTable(rest);
          if (restParsed) {
            nodes.push(...renderTable(restParsed, nodeKey++));
          } else {
      ...
      if (parsed) {
        nodes.push(...renderTable(parsed, nodeKey++));
        continue;
      }
...
/** Render a parsed GFM table (+ any trailing prose) as nodes. The
 *  table JSX was previously inline; extracted so headings that share
 *  a paragraph with a table can reuse it. */
function renderTable(
  parsed: { table: TableSpec; rest: string },
  key: number,
): ReactNode[] {
  const nodes: ReactNode[] = [];
  nodes.push(
    <table key={`tbl-${key}`} className={s.assistantTable}>
      <thead>
        <tr>
  ...
  // tryParseTable returns the unconsumed remainder for tables that
  // share a paragraph with trailing prose — render that as a
  // follow-on paragraph so nothing gets dropped.
  const rest = parsed.rest.trim();
  if (rest) {
    nodes.push(<p key={`p-${key}-rest`}>{renderInline(rest)}</p>);
  }
  return nodes;
}
